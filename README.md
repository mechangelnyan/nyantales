...
Build output: **200KB JS** (from 385KB), **97KB CSS** (from 124KB). Gzip'd: ~55KB JS, ~14KB CSS. GitHub Pages CI runs the build automatically. A **story manifest** (8KB JSON) replaces 30 individual YAML fetches on boot — title screen loads instantly without parsing 1.6MB of YAML.

**Testing:** 204 unit tests (Node.js) + 269 Playwright browser tests covering UI, a11y, persistence, deep links, campaign flow, engine logic, manifest/lazy-loading, and production builds. Run `npm test` and `npx playwright test`.
...
│   └── web/
│       └── vn.spec.js      269 Playwright browser tests for the web VN
├── package.json
└── README.md
```
